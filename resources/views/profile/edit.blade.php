    ...
    <div class="w-[99%]  bg-white  h-auto pb-4 flex flex-col md:flex-row justify-center">
        <div class="w-full md:w-1/2 h-[50vh] space-y-3 hidden md:flex flex-col justify-center items-center">
            <div class="w-60 h-60 rounded-full bg-gray-300">
                ...
                <img id="svg-img" src="{{asset('storage/profil/'.Auth::user()->image)}}" alt="photo de {{Auth::user()->name}}" class="w-full h-full rounded-full object-cover">
            </div>
                <div id="add-img" class="w-11/12  h-10 bg-[#8259fd] cursor-pointer text-white rounded-md flex justify-center items-center gap-1">
        ...
        <div class="w-full h-auto md:w-1/2 border-l flex flex-wrap space-y-4  justify-center md:justify-between">
            ...
            <form class="w-full flex justify-center" method="POST" action="{{ route('logout') }}">
                @csrf
